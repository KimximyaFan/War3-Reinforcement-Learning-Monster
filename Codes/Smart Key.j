library SmartCastSystem initializer Init requires DzAPIFrameHandle, DzAPIHardware, DzAPIPlus, MemoryLib
    globals
        ...
        boolean Smartkey_On_D = true
        boolean Smartkey_On_F = true
        boolean Smartkey_On_T = true
    ...
    private function HandleKeyPress takes nothing returns nothing
        ...
        elseif keycode==JN_OSKEY_D then
            if not Smartkey_On_D then
                return
            endif
        elseif keycode==JN_OSKEY_F then
            if not Smartkey_On_F then
                return
            endif
        elseif keycode==JN_OSKEY_T then
            if not Smartkey_On_T then
                return
            endif
        endif
        //-----------------------끝--------------------
    ...
    private function RegisterAlphabetKeys takes nothing returns nothing
        ...
        call RegisterKey(JN_OSKEY_D)
        call RegisterKey(JN_OSKEY_F)
        call RegisterKey(JN_OSKEY_T)
